...
public class CmsCenterHelper
{
    ...
    public static boolean getLibraryItems(Activity activity,
            SortOrder sortOrder, AscDescOrder ascOrder, EventedArrayList<GridItemData> result)
    {
        ArrayList<OnyxLibraryItem> items = new ArrayList<OnyxLibraryItem>();
        if (!OnyxCmsCenter.getLibraryItems(activity, sortOrder, ascOrder, items)) {
            return false;
        }
    ...
    public static boolean searchLibraryItem(Activity activity, String pattern,
     ...
     * @return
     */
    public static boolean getOrCreateMetadata(Activity activity,
            BookItemData item, RefValue<OnyxMetadata> result,
            RefValue<String> errMsg)
    {
     ...
     * @return
     */
    public static boolean updateRecentReading(Activity activity,
            BookItemData item, OnyxMetadata metadata)
    {
        String last_access = OnyxMetadata.DATE_FORMAT.format(new Date());
        Log.d(TAG, "updating recent reading: " + item.getText() + ", "
                + last_access);
    ...
    public static boolean removeRecentReading(Activity activity,
            BookItemData item)
    {
        if (sRecentReadings == null) {
            assert (false);
    ...
    /**
     * only for OnyxLauncher, using internal storing knowledge of thumbnail to
     * optimize read time
     * 
     * @param context
     * @param metadata
            ...
            RefValue<Bitmap> result)
    {
        ...
        String path = OnyxCmsProvider.getThumbnailFile(context,
                metadata.getMD5());
